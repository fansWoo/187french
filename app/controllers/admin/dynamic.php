...
class Dynamic extends CI_Controller {
	...
	public function postdynamic($do = '', $dynamicid = 0)
	{
		...
		if($do == 'post')
        {
            ...
            if ($this->form_validation->run() !== FALSE)
            {
                ...
                $this->dynamic_model->construct( array('dynamicid' => $dynamicid, 'title' => $title, 'classid_array' => $classid_array, 'content' => $content, 'prioritynum' => $prioritynum, 'modelname' => 'dynamic') );
                
                $this->dynamic_model->update();
		...
		else
		{
            ...
            $this->class_list_model->db_construct( array('db_where' => array('uid' => $data['user']['uid'], 'modelname' => 'dynamic') ) );
	...
	public function postclass($do = '', $classid = 0)
	{
		...
		if($do == 'post')
        {
            ...
            if ($this->form_validation->run() !== FALSE)
            {
                ...
                $this->class_model->construct( array('classid' => $classid, 'classname' => $classname, 'slug' => $slug, 'modelname' => 'dynamic') );
		...
		else
		{
            ...
            {
                $this->load->model('class_model');
                $this->class_model->db_construct( array('db_where' => array('classid' => $classid, 'modelname' => 'dynamic') ) );
	...
	public function classlist($input = '')
	{
		...
		else
		{
            ...
            $this->db->from('class');
            $this->db->where( array('uid' => $uid, 'status !=' => 0, 'modelname' => 'dynamic') );
            $query = $this->db->get();
	...
	public function set($input = '')
	{
		...
		if($input == 'post')
        {
			$this->form_validation->set_rules('dynamic_order', '文章顯示排序', 'required');
			$this->form_validation->set_rules('dynamic_amount', '文章顯示數量', 'required');
				
			if ($this->form_validation->run() !== FALSE)
			{
                $dynamic_order = $this->input->post('dynamic_order', TRUE);
                $this->load->model('setting_model');
                $this->setting_model->construct( array('keyword' => 'dynamic_order', 'value' => $dynamic_order, 'status' => 2) );
				$this->setting_model->update();
                
                $dynamic_amount = $this->input->post('dynamic_amount', TRUE);
                $this->setting_model->construct( array('keyword' => 'dynamic_amount', 'value' => $dynamic_amount, 'status' => 2) );
				$this->setting_model->update();
                
				$message = '設定成功';
				$url = 'admin/dynamic/set';
			}
			else
			{
				$message = '設定失敗';
		...
		else
		{
            ...
            $this->setting_list_model->db_construct( array('keyword' => array('dynamic_order', 'dynamic_amount') ) );
