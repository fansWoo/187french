...
<div class="contentBox allWidth">
    ...
    <?php echo form_open("admin/$child1_name_Str/$child2_name_Str/$child3_name_Str/{$child4_name_Str}_post/") ?>
        <div class="spanLine">
            <div class="spanLineLeft text width100">
                <input type="number" class="text" style="margin-left:-6px;" value="<?=!empty($search_courseid_Num)?$search_courseid_Num:''?>" name="search_courseid_Num" placeholder="請填寫ID">
            </div>
            <div class="spanLineLeft text width500">
    ...
    <?if(!empty($course_CourseList->obj_Arr)):?>
    <?foreach($course_CourseList->obj_Arr as $key => $value_Course):?>
    <div class="spanLine">
        <div class="spanLineLeft text width100">
            <?=$value_Course->courseid_Num?>
        </div>
        <div class="spanLineLeft text width500">
        ...
        <div class="spanLineLeft width300 hoverHidden">
            ...
            <span class="ahref" onClick="fanswoo.check_href_action('確定要刪除嗎？', 'admin/<?=$child1_name_Str?>/<?=$child2_name_Str?>/<?=$child3_name_Str?>/delete/?courseid=<?=$value_Course->courseid_Num?>&hash=<?=$this->security->get_csrf_hash()?>');">刪除</span>
        </div>
	</div>
    <?endforeach?>
    <?else:?>
    ...
    <?endif?>
    <div class="pageLink"><?=$course_links?></div>
</div>
<?=$temp['admin_footer']?>