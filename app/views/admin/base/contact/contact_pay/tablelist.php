...
<div class="contentBox allWidth">
    ...
    <?if(!empty($ContactPayList->obj_Arr)):?>
    <?foreach($ContactPayList->obj_Arr as $key => $value_ContactPay):?>
    <div class="spanLine">
        <div class="spanLineLeft text width100">
            <?=$value_ContactPay->contactid_Num?>
        </div>
        <div class="spanLineLeft text width200">
        ...
        <div class="spanLineLeft text width200">
            <?=$value_ContactPay->updatetime_DateTime->datetime_Str?>
        </div>
        <div class="spanLineLeft width300 hoverHidden">
            <a href="admin/<?=$child1_name_Str?>/<?=$child2_name_Str?>/<?=$child3_name_Str?>/edit/?contactid=<?=$value_ContactPay->contactid_Num?>">編輯</a>
            <span class="ahref" onClick="fanswoo.check_href_action('確定要刪除嗎？', 'admin/<?=$child1_name_Str?>/<?=$child2_name_Str?>/<?=$child3_name_Str?>/delete/?contactid=<?=$value_ContactPay->contactid_Num?>&hash=<?=$this->security->get_csrf_hash()?>');">刪除</span>
        </div>
	</div>
    <?endforeach?>
    <?else:?>
