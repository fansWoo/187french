...
<div class="contentBox allWidth">
    ...
    <?if(!empty($ContactLearnList->obj_Arr)):?>
    <?foreach($ContactLearnList->obj_Arr as $key => $value_ContactLearn):?>
    <div class="spanLine">
        <div class="spanLineLeft text width100">
            <?=$value_ContactLearn->contactid_Num?>
        </div>
        <div class="spanLineLeft text width200">
            <a href="admin/<?=$child1_name_Str?>/<?=$child2_name_Str?>/<?=$child3_name_Str?>/edit/?contactid=<?=$value_ContactLearn->contactid_Num?>"><?=$value_ContactLearn->name_Str?></a>
        </div>
        <div class="spanLineLeft text width200">
            <?if($value_ContactLearn->status_process_Num == 1):?>
            未處理
            <?elseif($value_ContactLearn->status_process_Num == 2):?>
            已處理
            <?endif?>
        </div>
        <div class="spanLineLeft text width200">
            <?=$value_ContactLearn->updatetime_DateTime->datetime_Str?>
        </div>
        <div class="spanLineLeft width300 hoverHidden">
            <a href="admin/<?=$child1_name_Str?>/<?=$child2_name_Str?>/<?=$child3_name_Str?>/edit/?contactid=<?=$value_ContactLearn->contactid_Num?>">編輯</a>
            <span class="ahref" onClick="fanswoo.check_href_action('確定要刪除嗎？', 'admin/<?=$child1_name_Str?>/<?=$child2_name_Str?>/<?=$child3_name_Str?>/delete/?contactid=<?=$value_ContactLearn->contactid_Num?>&hash=<?=$this->security->get_csrf_hash()?>');">刪除</span>
        </div>
	</div>
    <?endforeach?>
    <?else:?>
