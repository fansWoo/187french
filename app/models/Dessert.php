...
class Dessert extends ObjDbBase
{

    public $dessertid_Num = 0;
    public $uid_Num = 0;
    public $name_Str = '';
    public $name_french_Str = '';
    public $pic_PicObjList;//照片類別列表
    public $content_Str = '';//網頁語言類別
    public $price_Num = 0;
    public $class_ClassMetaList;//分類標籤類別列表
    public $prioritynum_Num = 0;
    public $updatetime_DateTime;
    public $status_Num = 1;
    public $db_name_Str = 'dessert';//填寫物件聯繫資料庫之名稱
    public $db_uniqueid_Str = 'dessertid';//填寫物件聯繫資料庫之唯一ID
    public $db_field_Arr = array(//填寫資料庫欄位與本物件屬性之關係，前者為資料庫欄位，後者為屬性
        'dessertid' => 'dessertid_Num',
        'uid' => 'uid_Num',
        'name' => 'name_Str',
	...
	{
        //引入引數並將空值的變數給予空值
        reset_null_arr($arg, ['dessertid_Num', 'uid_Num', 'name_Str', 'name_french_Str', 'picids_Str', 'picids_Arr', 'content_Str', 'price_Num', 'classids_Str', 'classids_Arr', 'prioritynum_Num', 'updatetime_Str', 'updatetime_inputtime_date_Str', 'updatetime_inputtime_time_Str', 'status_Num']);
        foreach($arg as $key => $value) ${$key} = $arg[$key];
        
        //將引數設為物件屬性，或將引數作為物件型屬性的建構值
        $this->set('dessertid_Num', $dessertid_Num);
