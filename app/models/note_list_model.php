...
class Note_list_model extends CI_Model {
	...
	public function construct($arg = array('note_array' => array() ) )
	{
        ...
        {
            $Note = new CI_Model();
            $modelname = 'note_model'.rand(0,32767);
            $Note->load->model('note_model', $modelname);
            $Note->$modelname->construct(array(
                'noteid' => $value['noteid'],
                'uid' => $value['uid'],
            ...
            $note_array = $Note->$modelname->get_array();
            $this->note_list_model[] = $Note;
